...
while i<len(res_list):
    ...
    aa[i][j+1]=round(math.sqrt(((aa[i][0][x]-aa[i][j])**2+(aa[i][1][x]-aa[i][j])**2+(aa[i][2][x]-aa[i][j])**2)/3),2)
    aa[i][j+2]=round((aa[i][0][y]+aa[i][1][y]+aa[i][2][y])/3,2)
    aa[i][j+3]=round(math.sqrt(((aa[i][0][y]-aa[i][j+2])**2+(aa[i][1][y]-aa[i][j+2])**2+(aa[i][2][y]-aa[i][j+2])**2)/3),2)
    print(aa[i])
    i=i+1
    j=0
#print('EN=x,EG=y\n',aa)
