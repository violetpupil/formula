...
while i<len(res_list):
    while j<len(num_list):
        ...
        j=j+1
    aa[i][j]=(aa[i][0][x]+aa[i][1][x]+aa[i][2][x])/3
    aa[i][j+1]=math.sqrt((aa[i][0][x]-aa[i][j])**2+(aa[i][1][x]-aa[i][j])**2+(aa[i][2][x]-aa[i][j])**2)
    aa[i][j+2]=(aa[i][0][y]+aa[i][1][y]+aa[i][2][y])/3
    aa[i][j+3]=math.sqrt((aa[i][0][y]-aa[i][j+2])**2+(aa[i][1][y]-aa[i][j+2])**2+(aa[i][2][y]-aa[i][j+2])**2)
    i=i+1
    j=0
print('EN=x,EG=y\n',aa)
